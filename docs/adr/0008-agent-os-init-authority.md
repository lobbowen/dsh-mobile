...
| 空壳上屏（¬BORN 必须出现在常驻通知上） | **已落并真机复点通过**（壳 1.1.7(9)，02:05 同型注入）：正文单写者 = `statusLine()` 后，t+24s 实读「常驻被打断：… · 运行时未出生 · 通道通 · :node 已绑定」，撤注入立刻回「运行时在线」；1.1.6(8) 上 13+4 个采样点一次都没印出过这句（定罪过程与口径更正见执行案 D9/§7） |
