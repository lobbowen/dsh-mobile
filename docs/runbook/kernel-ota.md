...
| 机制 | 说明 |
|---|---|
| **不删半包** | 下到 `kernel-ota-<ver>.zip.part`；失败或预算耗尽都**保留**（自检据此只报进度不判红，**唯一判红的是 0 字节空壳**——里面没有任何可续传的东西） |
...
### 3.7 安装暂存与开机清扫（半包保留 ≠ 尸体保留）

解包落在 `files/kernel/<ver>.tmp-<pid>-<毫秒>`，改名（rename）进正式版本目录才算装好。
一次被杀的安装会把这棵临时树留在设备上：真机 2026-09-26 实测 `0.1.0-android.12.tmp-*`
长期驻留，界面零痕迹，且被 `installedVersions()` 当成候选版本。

| 规则 | 说明 |
|---|---|
| **只在开机扫** | `NodeRuntimeService` 在 OTA 之前、每进程一次扫暂存命名。此刻不可能有活的安装（安装只由本进程在此之后发起），所以删它是安全的 |
| **只认暂存命名** | 正式版本目录与 `CURRENT`/`FLOOR`/`PENDING` 指针一律不碰；建名与认名共用 `KernelManager.stagingDirName` / `isStagingDir` 一个出处 |
| **删不掉要说** | 清扫返回「清掉的」与「删不掉的」两格，诊断按后者判红绿 —— 只报成功清单会把删除失败洗成「本来就没有」 |
| **与半包相反** | `.zip.part` 是**设计内保留**（续传进度），暂存目录是**设计外残留**（不可用），前者不删、后者必删 |
