...
- 状态：**v3** · 2026-09-26（v2 红线不变；v3 立门禁维护法）
...
## 7. 门禁法（v3 新增：门禁不许长成第二套源码）

背景：2026-09-26 全量审计，测试/门禁约 14,500 行 ≈ 源码的 42%。总量可控，但三类门禁的
**边际维护成本**在制造"双向开发"税：多副本判据（同一判据在 2–4 个 workflow 各存一份，
改一处必须同步全部，且已实证漂移）、钉字面量/文件名的扫描、需要人记得同步的手维护清单。
以下四条约束所有**新增或整改**的门禁；行为回归与防复活绊线是资产不是负债，不在收敛之列。

1. **一判据一实现**：门禁逻辑住 `scripts/`（先例：`verify-runtime-elf.sh`），workflow 只准调用、
   不准内联判据正文。同一判据出现第二份拷贝 = 缺陷，第二份必须改为转调或删除。
2. **判据数据化，清单禁手维护**：新增门禁优先做成"数据格子 + 唯一执行器"（先例：
   `kernel/src/guard/native/supply-table.json` 每格 `verify` + `capability-probe.js`）。
   必须进 git 的清单只能由生成器产出、并被对账门禁核验（先例：`gen-native-assets.js` ↔ `git diff --exit-code`）。
3. **扫描式门禁限额 + 自证**：绊线/词汇类门禁 ≤60 行，必须带违例样本自证（能构造命中使它在 CI 红）；
   超限额的扫描门禁按"第二套源码"对待，必须拆成数据 + 执行器。
   判据不许钉注释内容或可改名标识符——钉行为契约（PATH 门禁被注释字面量撞计数是已发生过的事故）。
4. **门禁预算制（一进一退）**：净增 >100 行的门禁 PR，必须同时退役或合并 ≥ 等量的重叠覆盖门禁；
   做不到就在 PR 说明里写清为什么。目的：阻断"每烧一个缺陷加一个文件"的只进不出棘轮
   （`kernel/test/` 攒到 74 个文件就是这个机制缺失的结果）。
