...
| ⑦ | 常驻形态（2026-09-26 新立，用户拍板「升前台占一条通知」）：锁屏 5 分钟后解锁 | 监督者的状态通知（id 1004）仍在、内容含「运行时/通道/:node」三要素，点按进首页；期间 :node 未被清 | **待定罪**。代码侧的常驻边由门禁 `KEEP_ALIVE_EDGES` 逐条实名钉；ROM 是否允许 specialUse 长期驻留只能真机验 |
...
| ⑪ | 被杀即定罪（2026-09-26 拍板：不做进程外复活，只让打断可见）：设置里「强行停止」后重开 | 通知首行 + 首页第一行 + 导出报告都写「常驻被打断：上次存活到 HH:mm:ss，中断 …」三处同句式；`am stop-service` 后重开**不该**出现这一行（证明 clean 戳真的在写）；整机重启后首启写「结束于设备重启」而非「App 被回收」 | **待定罪**。复活边已整体删除并列入门禁 DEAD 词汇（防回潮） |
...
- G1（v2 修订，缺一不可）：
  ...
  6. **运行时是底座不是步骤**（2026-09-26 新增）：不进任何界面、只解锁屏幕，控制面就应在线
     （靠常驻边，不靠任何"复活"机制）；锁屏 5 分钟不丢常驻通知；界面上没有拆运行时的按钮
     （§7⑦⑩、flow-spec §6-8）。
  7. **配对不是黑盒**（2026-09-26 新增）：每一次输码送达都在通知与首页「最近动作」留下同句式结论
     （§7⑧、flow-spec §6-9）。「试了很多次突然成功」= 本条判据未满足，不算交付。
  8. **打断不是暗账**（2026-09-26 补）：进程被系统回收过一次，就必须有一次看得见的定罪
     （§7⑪）。被杀后静默地把壳点回来而通知仍写「运行时在线」= 本条判据未满足，不算交付。
