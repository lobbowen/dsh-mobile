...
function shell(o) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const logs = [];
    // 超时必须以错误收口：resolve 部分输出会让挂死的会话被上层报成"成功但空输出"。
    const timer = setTimeout(() => { logs.push('TIMEOUT'); finish(new Error('adb shell 超时（' + timeoutMs + 'ms），输出不完整' )); }, timeoutMs);
    function finish(err) {
...
module.exports = { shell, makePacket, checksum, commandInt, VERSION, MAX_PAYLOAD, HEADER_SIZE };
