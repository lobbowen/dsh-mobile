 ...
 *
 * 职责（对应 docs/contracts/bridge-protocol.md §3.2 的六个方法）：
 ...
 * ui.waitFor → [waitForNode] 轮询节点出现（由 HostBridgeService 驱动）
...
class DshAccessibilityService : AccessibilityService() {
     ...
     * 供文本注入/焦点定位用；注意 ui.waitFor 轮询走 rootInActiveWindow（见 waitForNode），
     * 看不到 IME/悬浮窗里的节点 —— 这是已知盲区，别再靠注释把两者混为一谈。
     */
